...
class EmployeePortalController extends Controller
{
    ...
    public function leave()
    {
        ...
        $leave = Leave::where('emp_id', \Auth::user()->id)->orderby('id', 'desc')->get();

        $year_active = DB::table('sys_leave_setup')->value('year_active');

        $leave_ledger = LeaveLedger::where('employee_id', \Auth::user()->id)->where('year',$year_active)->orderby('id', 'desc')->first();
        ...
        try {
           $balance_sl = $leave_ledger->sl_balance;
           $balance_vl = $leave_ledger->vl_balance;
           $total_balance = $balance_sl + $balance_vl;
        } catch (Exception $e) {
            $balance_sl = 0;
            $balance_vl =0;
            $total_balance = 0;
        ...
        return view('leave.leave-employee', compact('leave', 'balance_sl', 'balance_vl','total_balance','leave_type'));
    }
    ...
    public function postExpense(Request $request)
    {
        ...
        if ($bill_copy != '') {
            ...
            $bill_copy_name = $bill_copy->getClientOriginalastname();
    ...
    public function postTicket(Request $request)
    {
        ...
        $cl_name = $cl->firstname . ' ' . $cl->lastname;
        $cl_email = $cl->email;
        ...
        $conf = EmailTemplate::where('tplastname', '=', 'Ticket For Admin')->first();
    ...
    public function replayTicket(Request $request)
    {
        ...
        $conf = EmailTemplate::where('tplastname', '=', 'Employee Ticket Reply')->first();
        $estatus = $conf->status;
