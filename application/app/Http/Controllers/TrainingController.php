...
class TrainingController extends Controller
{
    ...
    public function employeeTraining()
    {
        ...
        if (session('portal')=="master") {
            ...
            $emp_training=EmployeeTraining::where('employee_id',\Auth::user()->id)->orderBy('id', 'desc')->get();
        }
    ...
    public function postNewTraining(Request $request)
    {
        ...
        $v=\Validator::make($request->all(),[
            'employee_id'=>'required','training_title'=>'required'
        ]);
        ...
        $emp_training->training_hours=$request->hours;
        $emp_training->employee_id=$request->employee_id;
        $emp_training->save();
    ...
    public function postEmployeeTrainingInfo(Request $request)
    {
      ...
      public function trainingNeedsAssessment()
      {
        ...
        if (session('portal')=="master") {
            ...
            $tnassessment=TrainingNeedsAssessment::where('employee_id',\Auth::user()->id)->get();
        }
    ...
    public function postNewTrainingNeedsAssessment(Request $request)
    {
        ...
        $v=\Validator::make($request->all(),[
            'employee_id'=>'required','training_title'=>'required','status'=>'required'
        ]);
        ...
        $target_completion=date('Y-m-d',strtotime($request->target_completion));

        $emp_training=new TrainingNeedsAssessment();

        $emp_training->employee_id=$request->employee_id;
        $emp_training->title=$request->training_title;
        $emp_training->development_area=$request->development_area;
        $emp_training->target_completion=$target_completion;
        $emp_training->priority=$request->priority;
        $emp_training->responsible_person=$request->responsible_person;
        ...
        return redirect('training/training-needs-assessment')->with([
            'message' => "Training needs added successfully"
        ]);
    ...
    public function postTrainingNeedsAssessmentUpdate(Request $request){
        ...
        if ($emp_training){
            ...
            $emp_training->responsible_person=$request->responsible_person;
            $emp_training->training_reason=$request->training_reason;
            $emp_training->hours=$request->hours;
            $emp_training->status=$request->status;

            $emp_training->save();

            if ($request->status=="completed"){
                EmployeeTraining::where('event_id',$cmd)->delete();
                    $emp_training = new EmployeeTraining;
                    $emp_training->event_id= $cmd;
                    $emp_training->employee_id = $request->employee_id;
                    $emp_training->title=$request->training_title;
                    $emp_training->training_location=$request->training_location;
                    $emp_training->sponsored_by=$request->sponsored_by;
    ...
    public function trainingEvents(){
        ...
        if (session('portal')=="master") {
            ...
            $training_event=TrainingEvents::where('status','upcoming')->get();
        }
    ...
    public function postNewTrainingEvent(Request $request){
        ...
        $v=\Validator::make($request->all(),[
            'employee'=>'required','training_title'=>'required','status'=>'required'
        ]);
    ...
    public function postTrainingEventUpdate(Request $request){
        ...
        if ($training_event){
            ...
            $training_event->status=$request->status;
            $training_event->description=$request->description;
            $training_event->hours=$request->hours;
            $training_event->save();
    ...
    public function TrainingEvaluations(){
        ...
        if (session('portal')=="master") {
            ...
            $training=EmployeeTraining::orderBy('title')
                ->groupBy('title')
                ->get();
        }else{
            $training_evaluation=TrainingEvaluations::where('employee_id',\Auth::user()->id)->get();
            $training=EmployeeTraining::where('employee_id',\Auth::user()->id)->get();
        }
